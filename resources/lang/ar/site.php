<?php
    return[

        'dashboard'=>'الرئيسية',
        'Contact'=>'تواصل معنا',
        'Search'=>'بحث',
        'Alexander Pierce'=>'الكسندر بيرس',
        'CPU Traffic'=>'حركة وحدة المعالجة المركزية',
        'Likes'=>'الإعجابات',
        'Sales'=>'مبيعات',
        'New Members'=>'أعضاء جدد',


        'users'=>'المشرفين',
        'first_name'=>'الإسم الأول',
        'last_name'=>'الإسم الأخير',
        'email'=>'البريد الإلكترونى',
        'action'=>'أكشن',
        'no_deta_found'=>'للاسف لايوجد سجلات',
        'edit'=>'تعديل',
        'delete'=>'حذف',
        'update'=>'تعديل',
        'add'=>'اضف',
        'read'=>'عرض',
        'create'=>'اضافه',


        "password"=>'كلمه المرور',
        'password_confirmation'=>'تأكيد كلمه المرور',
        'added_successfully'=>'تم اضافه البيانات بنجاح',
        'deleted_successfully'=>'تم حذف البيانات بنجاح',
        'updated_successfully'=>'تم تعديل البيانات بنجاح',

        'permissions'=>'الصلاحيات',
        'categories'=>'الاقسام',
        'products'=>'المنتجات',
        'logout'=>'تسجيل الخروج',
        'yes'=>'نعم',
        'no'=>'لا',
        'confirm_delete'=>'تأكيد الحذف',
        'image'=>'الصوره',
        'remember_me'=>'تذكرنى',
        'login'=>'تسجيل الدخول',
        'name'=>'الاسم',

        'name_ar' => 'الاسم بالغه العربيه',
        'name_en' => 'الاسم بالغه الانجليزيه',

        'descraption_ar' =>'الوصف بالغه العربيه',
        'descraption_en' =>'الوصف بالغه الانجليزيه',
        'all_categories' =>'كل الاقسام',

        'purchase_price' =>'سعر الشراء',
        'sale_price' =>'سعر البيع',
        'stock' =>' المخزون',
        'titel_ar' => 'العنوان بالغه العربيه',
        'titel_en' => 'العنوان بالغه الانجليزيه',

        'category_name'=>'القسم',
        'prodact_name'=>'الاسم',
        'desc_name'=>'الوصف',
        'profit_percent'=>'المكسب',

        'products_count'=>'عدد المنتجات',
        'related_products'=>'المنتجات المرتبطه',


        'clients'=>'العملاء',
        'phone'=>"التليفون",
        'address'=>'العنوان',

        'orders'=>'طلبات',
        'add_order'=>'اضف طلب',
        'edit_order' => 'تعديل طلب',

        'price'=>'السعر',
        'product'=>'المنتج',
        'quantity'=>'الكميه',
        'no_records'=>'لايوجد سجلات',
        'total'=>'المجموع',

        'show_products' => 'عرض المنتجات',
        'print' => 'طبع',
        'client_name' => 'اسم العميل',
        'created_at' => 'تم اضافته',
        'show' => 'عرض',
        'loading'=>'جارى التحميل',
        'previous_orders' => 'الطلبات السابقه',
        'Sales_Graph'=>'الرسم البياني للمبيعات',


        'ar'=>'بالغه العربية',
        'en'=>'بالغة الانجليزية',
        'AR'=>'بالغه العربية',
        'EN'=>'بالغة الانجليزية',
        'FR'=>'بالغه الفرنسية',
        'fr'=>'بالغه الفرنسية',

















    ];
?>